          ...
          <div className="grid lg:grid-cols-[1fr_auto] gap-10 items-end">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 text-white/70">
                High-quality building materials, finishing products and hardware solutions for every construction and renovation need.
              </p>
            </div>
      ...
      <section className="py-12 md:py-16">
        <div className="container-x grid lg:grid-cols-[240px_1fr] gap-10">
          ...
          <aside className="space-y-6">
            <div>
              <h3 className="text-xs uppercase tracking-widest font-bold text-orange mb-3">Product Categories</h3>
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 justify-between mb-6">
              <p className="text-sm text-muted-foreground">
                Showing <span className="font-bold text-charcoal">{filtered.length}</span> of {products.length}+ products
              </p>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex border border-border rounded-md overflow-hidden bg-white">
                  <button onClick={() => setView("grid")} className={`p-2 ${view === "grid" ? "bg-orange text-white" : "text-charcoal"}`} aria-label="Grid">
                    <Grid3x3 className="h-4 w-4" />
                  </button>
                  <button onClick={() => setView("list")} className={`p-2 ${view === "list" ? "bg-orange text-white" : "text-charcoal"}`} aria-label="List">
            ...
            {view === "grid" ? (
              ...
              <div className="space-y-3">
                {filtered.map((p) => (
                  <div key={p.id} className="flex gap-4 border border-border rounded-md p-3 bg-white hover:border-orange transition-colors">
                    <img src={p.image} alt={p.name} loading="lazy" className="w-28 h-28 object-cover rounded" />
